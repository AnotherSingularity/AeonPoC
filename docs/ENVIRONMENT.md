...
Aeon is sensitive to the torch/CUDA build because the training forward (per-token
at K=1, the training default) holds the whole sequence's activation graph. Use
the pinned stack below; do not let `pip` pull a default-PyPI torch newer than
what's pinned.
...
Gradient checkpointing for the per-token (K<seq_len) training forward is queued as
a **Layer 4** follow-up (see `docs/AEON_V2_CLEANUP_PLAN.md`): wrap the
per-token block stack in `torch.utils.checkpoint.checkpoint(use_reentrant=False)`,
thread the recurrent state as explicit input/output (not via `self`), and keep
`DynamicCache.update()` outside the checkpoint boundary so the backward recompute
does not double-append. Reference: arXiv 2008.07027. Expected ~50–70%
activation-memory reduction at ~20–30% throughput cost. This reduces *activation*
memory and is orthogonal to the cu130 *baseline*-memory regression above —
pinning torch is the actual fix for the OOM we hit.
